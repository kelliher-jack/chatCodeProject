...
class Column {
 ...
 public:
    ...
    void RegionSetPosition(Point value);

    std::vector < Segment * > GetDistalMatchingSegments();

    Point GetHypercolumnPosition() {
	return HypercolumnPosition;
    }
    ...
    float GetFastActiveDutyCycle() {
	return FastActiveDutyCycle;
    }

    float GetOverlapDutyCycle() {
	return _overlapDutyCycle;
    }
    ...
    int GetMinOverlapToReuseSegment() {
	return MinOverlapToReuseSegment;
    }
    ...
    Cell *GetCellByIndex(int _index) {
	return Cells[_index];
    }
    ...
    void SetIsPredicted(bool value) {
	IsPredicted = value;
    }  
    ...
    void SetIsInhibited(bool value) {
	IsInhibited = value;
    }
    ...
    /// Initializes a new Column for the given parent Region
    /// at source row/column position srcPos and column grid position pos.
    /// 
    Column(Region * _region, Point pos, int minOverlapToReuseSegment);
    Column(Region * _region, Point pos);    
    ...
    void CreateProximalSegments(std::vector < DataSpace * >&inputList,
				std::vector < int >&inputRadii);
    void CreateProximalSegments(std::vector < DataSpaceRef * >&refList,
				std::vector < int >&inputRadii);
    ...
    /// type 0 specifies normal spatial pooler,  1 for union pooler
