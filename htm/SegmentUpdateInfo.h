...
/// the radius.
class SegmentUpdateInfo:public MemObject {
 public:
    SegmentUpdateInfo(void);
    ~SegmentUpdateInfo(void);

    MemObjectType GetMemObjectType() {
	return MOT_SEGMENT_UPDATE_INFO;
    }
    void Retire();
    ...
    DistalSynapse *createNewSynapse(Cell *presynapticCell, SynapseParameters *params);
    ...
    bool GetAddNewSynapses() {
	return AddNewSynapses;
    }
    void SetAddNewSynapses(bool value) {
	AddNewSynapses = value;
    }
    ...
    ///
    void Initialize(Cell * _cell, Segment * _segment,
		    FastList * activeDistalSynapses, bool addNewSynapses,
		    int _creationTimeStep, UpdateType _updateType);
