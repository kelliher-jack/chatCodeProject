...
#ifdef debug
	cout << "MEM: GetObject type " << _object_type;
	cout << " addr " << curObject << "\n";
#endif
...
#ifdef debug
    cout << "MEM: ReleaseObject type " << object_type;
    cout << " addr " << _object << "\n";
#endif
...
#endif	    
	case MOT_NEURON:
	    delete[](Neuron *) (cur_node->chunk);
	    break;
	case MOT_NSYNAPSE:
	    delete[](nSynapse *) (cur_node->chunk);
	    break;
	case MOT_COLUMN:
	    delete[](nColumn *) (cur_node->chunk);
	    break;
	}
...
void MemManager::NewChunk(MemObjectType _object_type)
{
	...
	for (int i = 0; i < FAST_HASH_TRAY_CHUNK_LENGTH; i++) {
	    ((FastHashTray *) (new_node->chunk))[i].mem_next =
		freeListArray[_object_type];
	    freeListArray[_object_type] =
		&(((FastHashTray *) (new_node->chunk))[i]);
	}
	...
	for (int i = 0; i < FAST_LIST_TRAY_CHUNK_LENGTH; i++) {
	    ((FastListTray *) (new_node->chunk))[i].mem_next =
		freeListArray[_object_type];
	    freeListArray[_object_type] =
		&(((FastListTray *) (new_node->chunk))[i]);
	}
	...
	for (int i = 0; i < PROXIMAL_SYNAPSE_CHUNK_LENGTH; i++) {
	    ((ProximalSynapse *) (new_node->chunk))[i].mem_next =
		freeListArray[_object_type];
	    freeListArray[_object_type] =
		&(((ProximalSynapse *) (new_node->chunk))[i]);
	}
	...
	for (int i = 0; i < DISTAL_SYNAPSE_CHUNK_LENGTH; i++) {
	    ((DistalSynapse *) (new_node->chunk))[i].mem_next =
		freeListArray[_object_type];
	    freeListArray[_object_type] =
		&(((DistalSynapse *) (new_node->chunk))[i]);
	}
	...
	for (int i = 0; i < SEGMENT_UPDATE_INFO_CHUNK_LENGTH; i++) {
	    ((SegmentUpdateInfo *) (new_node->chunk))[i].mem_next =
		freeListArray[_object_type];
	    freeListArray[_object_type] =
		&(((SegmentUpdateInfo *) (new_node->chunk))[i]);
	}
	...
	for (int i = 0; i < SEGMENT_CHUNK_LENGTH; i++) {
	    ((Segment *) (new_node->chunk))[i].mem_next =
		freeListArray[_object_type];
    ...
    case MOT_NSYNAPSE:
	// Create the new chunk
	new_node->chunk = new nSynapse[SYNAPSE_CHUNK_LENGTH];

	// Add all of the new chunk's objects to the free list
	for (int i = 0; i < SYNAPSE_CHUNK_LENGTH; i++) {
	    ((nSynapse *) (new_node->chunk))[i].mem_next =
		freeListArray[_object_type];
	    freeListArray[_object_type] =
		&(((nSynapse *) (new_node->chunk))[i]);
	}
	freeCountArray[_object_type] += SYNAPSE_CHUNK_LENGTH;
	countArray[_object_type] += SYNAPSE_CHUNK_LENGTH;
	break;
	
    case MOT_NSEGMENT:
	// Create the new chunk
	new_node->chunk = new nSegment[SEGMENT_CHUNK_LENGTH];

	// Add all of the new chunk's objects to the free list
	for (int i = 0; i < SEGMENT_CHUNK_LENGTH; i++) {
	    ((nSegment *) (new_node->chunk))[i].mem_next =
		freeListArray[_object_type];
	    freeListArray[_object_type] = &(((nSegment *) (new_node->chunk))[i]);
	}
	freeCountArray[_object_type] += SEGMENT_CHUNK_LENGTH;
	countArray[_object_type] += SEGMENT_CHUNK_LENGTH;
	break;
    ...
    case MOT_NEURON:
	// Create the new chunk
	new_node->chunk = new Neuron[NEURON_CHUNK_LENGTH];

	// Add all of the new chunk's objects to the free list
	for (int i = 0; i < NEURON_CHUNK_LENGTH; i++) {
	    ((Neuron *) (new_node->chunk))[i].mem_next =
		freeListArray[_object_type];
	    freeListArray[_object_type] = &(((Neuron *) (new_node->chunk))[i]);
	}
	freeCountArray[_object_type] += NEURON_CHUNK_LENGTH;
	countArray[_object_type] += NEURON_CHUNK_LENGTH;
	break;

    case MOT_COLUMN:
	// Create the new chunk
	new_node->chunk = new nColumn[COLUMN_CHUNK_LENGTH];

	// Add all of the new chunk's objects to the free list
	for (int i = 0; i < COLUMN_CHUNK_LENGTH; i++) {
	    ((Neuron *) (new_node->chunk))[i].mem_next =
		freeListArray[_object_type];
	    freeListArray[_object_type] = &(((nColumn *) (new_node->chunk))[i]);
	}
	freeCountArray[_object_type] += COLUMN_CHUNK_LENGTH;
	countArray[_object_type] += COLUMN_CHUNK_LENGTH;
	break;
    }    
}
