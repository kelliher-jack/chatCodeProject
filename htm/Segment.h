...
// Segments can be either proximal or distal (for spatial pooling or temporal
// pooling respectively) however the class object itself does not need to know
// which it ultimately is as they behave identically.  Segments are considered
// 'active' if enough of its existing synapses are connected and individually
// active.

/// Maximum number of prediction steps to track.
const int MaxTimeSteps = 10;

class Segment:public MemObject {
 public:
    Segment(void);
    ~Segment(void);
 ...
 private:
    // moved to expose everything simply to make msg passing easy to
    // load/copy opbjects for a quick prototype.  Note the way to do
    // this should write a routine to output/input data to object. JSK
 public:
    bool IsActive, WasActive, IsSequence;
    int ActiveConnectedSynapsesCount, PrevActiveConnectedSynapsesCount;
    int ActivePredictedConnectedSynapsesCount;
    int ActiveLearningSynapsesCount, PrevActiveLearningSynapsesCount;
    int InactiveWellConnectedSynapsesCount;
    int CreationTime;
    // for new temporal memory
    int prevNumActivePotentialSynapses, numActivePotentialSynapses;  
    ...
    void Initialize(int creationTime, float activeThreshold, float learningThreshold);  
    ...
    ProximalSynapse *CreateProximalSynapse(SynapseParameters * params,
    ...
    /// initPerm: the initial permanence of the synapse.
    /// Returns the newly created synapse.
    DistalSynapse *CreateDistalSynapse(SynapseParameters * params,
				       Cell * inputSource, float initPerm);
    ...
    int GetActiveConnectedSynapseCount() {
	return ActiveConnectedSynapsesCount;
    }

    int GetActivePredictedConnectedSynapseCount() {
      return ActivePredictedConnectedSynapsesCount;
    }

    /// Return a count of how many connected synapses on this segment 
    /// were active in the previous time step.
    int GetPrevActiveConnectedSynapseCount() {
	return PrevActiveConnectedSynapsesCount;
    }

    /// Return a count of how many synapses on this segment are from active learning cells
    /// in the current time step.
    int GetActiveLearningSynapseCount() {
	return ActiveLearningSynapsesCount;
    }

    /// Return a count of how many synapses on this segment were from active learning cells
    /// in the previous time step.
    int GetPrevActiveLearningSynapseCount() {
	return PrevActiveLearningSynapsesCount;
    }

    // Return a count of how many synapses on this segment are connected above InitialPerm, but are not active.
    int GetInactiveWellConnectedSynapsesCount() {
	return InactiveWellConnectedSynapsesCount;
    }
