...
void FastHashTable::Insert(int _key, void *_object)
{
	...
	if (((*tray_ptr)->key == _key)
	    && ((*tray_ptr)->pointer == _object)) {
    ...
    FastHashTray *new_tray =
	(FastHashTray *) (mem_manager.GetObject(MOT_FAST_HASH_TRAY));
...
void *FastHashTable::Get(int _key)
{
    ...
    for (FastHashTray * cur_tray = buckets[bucket_index];
	 ((cur_tray != NULL) && (cur_tray->key <= _key));
	 cur_tray = cur_tray->next) {
...
void *FastHashTable::Iterator_Advance()
{
	...
	while ((iterator_bucket_index < NUM_BUCKETS)
	       && (buckets[iterator_bucket_index] == NULL)) {
...
void *FastHashTable::Iterator_AdvanceForKey()
{
	...
	&& (iterator_cur->next->key == iterator_cur->key)) {
	iterator_cur = iterator_cur->next;
	return iterator_cur->pointer;
    } else {
