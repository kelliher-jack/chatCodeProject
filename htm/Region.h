...
#include "UnionTemporalPooler.hpp"
...
///    their are an estimated 200,000,000 minicolumns in the human brain
///    with 100 to 110 cells per column (6 layers, 20 per layer)
///    using a 32 bit integer can enumerate double the columns in a brain
///    but in reality both hyper-columns of upto maybe 10x10 and regions
///    of simular sensory/motor/simatic processing insure that 32 bits
///    beyond what would be needed to represent a single region composed
///    of hyper-columns of mini-columns and indexing all mini-columns.
///    16 bits 65,536 may also be sufficient esp for row/column indexes

class Region:public DataSpace {
  public:
    ~Region(void);
    ...
    nupic::algorithms::spatial_pooler::SpatialPooler spatialPooler;
    nupic::algorithms::union_temporal_pooler::UnionTemporalPooler unionTemporalPooler;
#ifdef APICALTIEBREAK
    nupic::experimental::apical_tiebreak_temporal_memory::ApicalTiebreakSequenceMemory temporalMemory;
#else
    nupic::experimental::apical_tiebreak_temporal_memory::ApicalTiebreakPairMemory temporalMemory;
#endif
    ...
    /// handle more successfully).  Passing in -1 for input radius will mean no restriction.
      Region(NetworkManager * manager, QString & _id, int uid, RegionType type,
