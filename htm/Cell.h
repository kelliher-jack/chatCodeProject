...
class Cell:public MemObject {
 ...
 public:
    ...
    // New Temporal Memory Code
    SegmentUpdateInfo *growNewSegment(int time);
    ...
    void SetWasLearning(bool value) {
	WasLearning = value;
    }
    ...
    bool GetIsSegmentPredicting() {
	return IsSegmentPredicting;
    }
    void SetIsSegmentPredicting(bool value) {
	IsSegmentPredicting = value;
    }
	...
	return NumPredictionSteps;
    }
    int GetPrevNumPredictionSteps() {
	return PrevNumPredictionSteps;
    }

    int GetPrevActiveTme() {
	return PrevActiveTime;
    }

    int GetAllStatus() {
        return ((IsActive & 0x01) | ((WasActive << 1) &0x02) |
		((IsLearning << 2) & 0x04) | ((WasLearning << 3) & 0x08) |
		((_isPredicting << 4) & 0x10) | ((WasPredicted << 5) & 0x20) |
		((IsSegmentPredicting << 6) & 0x40));
    }  
    ...
    SegmentUpdateInfo *UpdateSegmentActiveSynapses(bool previous,
						   Segment * segment,
						   bool newSynapses,
